...
namespace Tbasic.Types
{
    ...
    internal static class TypeConvert
    {
        ...
        private static bool TryConvertNonStrict<T>(object obj,  out T result, bool parseStrings = true, bool fastMath = true)
        {
            result = default(T);
            if (parseStrings) {
                string str = obj as string; // maybe we can convert it from a string?
                if (str != null) {
                    obj = ConvertFromString(str, fastMath);
                    if (obj == null)
                        return false;
            ...
            if (obj is IConvertible) {
                try {
                    ...
                    if (typeof(T).IsEnum) {
                        ...
                        if (n != null) {
                            return TryConvert(n.ForceToInt(), out result, strict: true, parseStrings: false); // if we don't turn on strict, we'll have infinite recursion 8/22/16
                        }
                    }
                    return false;
                }
        ...
        private static bool TryConvertNonStrict(object obj,  Type type, out object result, bool parseStrings = true, bool fastMath = true)
        {
            result = default(object);
            if (parseStrings) {
                string str = obj as string; // maybe we can convert it from a string?
        ...
        internal static object ConvertFromString(string str, bool fast)
        {
            // now we've just got to parse the supported types until we find a match...
            INumber n;
            if (Number.TryParse(str, out n, fast ? ExecuterOption.None : ExecuterOption.PreciseNumbers))
                return n;
            bool b;
